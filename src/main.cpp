...
void setup()
{
    ...
    auto main_engine_revolutions = ConnectTachoSender(kDigitalInputPin1, "0");
    ...
    auto main_engine_exhaust_temperature_metadata =
        new SKMetadata("K",                          // units
                       ...
                       "Engine Exhaust Temperature", // description
                       "Exhaust Temperature",        // short name
                       10.                           // timeout, in seconds
        );

    auto main_engine_revolutions_metadata =
    ...
    // connect temp sensors to Signal K output paths

    main_engine_oil_temperature->connect_to(new SKOutput<float>(
        "propulsion.0.oilTemperature", "/mainEngineOilTemp/skPath",
        main_engine_oil_temperature_metadata));
    main_engine_coolant_temperature->connect_to(new SKOutput<float>(
        "propulsion.0.temperature", "/mainEngineCoolantTemp/skPath",
        main_engine_coolant_temperature_metadata));
    main_engine_exhaust_temperature->connect_to(new SKOutput<float>(
        "propulsion.0.exhaustTemperature", "/mainEngineExhaustTemp/skPath",
    ...
    main_engine_oil_temperature->connect_to(new LambdaConsumer<float>(
        [](float temperature)
        { PrintTemperature(2, "Oil", temperature); }));
    main_engine_coolant_temperature->connect_to(new LambdaConsumer<float>(
        [](float temperature)
        { PrintTemperature(3, "Coolant", temperature); }));
    main_engine_exhaust_temperature->connect_to(new LambdaConsumer<float>(
        [](float temperature)
        { PrintTemperature(4, "Exhaust", temperature); }));
    main_engine_revolutions->connect_to(new LambdaConsumer<float>(
        [](float value)
        { PrintValue(display, 5, "RPM", 60 * value); }));
